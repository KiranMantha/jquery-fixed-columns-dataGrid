...
(function ($) {
    ...
    DataGrid.prototype = {
        ...
        _setDataChangeListener: function () {
            ...
            this.grid.fbDiv.find('tr td input').on('change', function () {
                var cellindx = $(this).parent()[0].cellIndex,
                    rowindx = $(this).closest('tr')[0].rowIndex;
            ...
            this.grid.fbDiv.find('tr td select').on('change', function () {
                var cellindx = $(this).parent()[0].cellIndex,
                    rowindx = $(this).closest('tr')[0].rowIndex;
                _this.settings.table.find('tbody tr:nth-child(' + (rowindx + 1) + ') td').eq(cellindx).find('select').val($(this).val());
            });
                ...
                $(".dgBody tbody").sortable({
                    ...
                    stop: function (event, ui) {
                        if (_this.settings.left > 0) {
                            post = ui.item.index();
                            other = $('.dgBody tbody').not(ui.item.parent());
                            //Use insertBefore if moving UP, or insertAfter if moving DOWN
                            if (post > pre) {
                                other.find('tr:eq(' + pre + ')').insertAfter(other.find(' tr:eq(' + post + ')'));
                            } else {
                                other.find('tr:eq(' + pre + ')').insertBefore(other.find(' tr:eq(' + post + ')'));
                            }
                        }
                ...
                this.grid.fbDiv.find('tr').each(function (i, row) {
                    $(row).on('change keyup', 'input', function (e) {
                        var type = e.currentTarget.type,
                            colindex = $(e.currentTarget).closest('td').index() + 1,
        ...
        _fixLeft: function () {
            ...
            var ftable = $('<table>').addClass($(this.settings.table)[this.useProp ? 'prop' : 'attr']('class')).addClass('dgFixedTable').css({
                'width': '1px',
                'table-layout': 'fixed',
                'background-color': '#fff',
